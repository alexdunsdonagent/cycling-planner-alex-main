#!/usr/bin/env python3
"""
One clean fix for everything:
1. Restores App.jsx from the best available backup
2. Patches src/data/translations.js with all new i18n keys
3. Verifies the result parses cleanly
"""
import re, shutil, os, sys

# ── Step 1: Restore App.jsx from best backup ─────────────────────
APP = "src/App.jsx"
backups = [
    "src/App.jsx.pre-i18n2",
    "src/App.jsx.pre-i18n",
...
for bak in backups:
    if os.path.exists(bak):
        shutil.copy(bak, APP)
        print(f"✅ App.jsx restored from {bak}")
        restored = True
        break
if not restored:
    print("⚠  No App.jsx backup found — leaving as-is")

# ── Step 2: Find translations file ───────────────────────────────
TRANS = "src/data/translations.js"
if not os.path.exists(TRANS):
    # T is still inline in App.jsx
    TRANS = APP
    print(f"ℹ  Using {TRANS} for translation keys")
else:
    print(f"✅ Found {TRANS}")
    shutil.copy(TRANS, TRANS + ".bak")

with open(TRANS, "r", encoding="utf-8") as f:
    src = f.read()

# ── Step 3: Find language block end by brace counting ────────────
def find_lang_end(src, lang):
    pat = re.compile(r'(?<![a-zA-Z])' + re.escape(lang) + r'\s*:\s*\{')
    m = pat.search(src)
    if not m: return None
    pos = m.end() - 1
    depth = 0; i = pos; in_str = False; sc = None
    while i < len(src):
        c = src[i]
        if in_str:
            if c == '\\': i += 2; continue
